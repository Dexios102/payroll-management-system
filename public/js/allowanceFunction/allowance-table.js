...
function exportToExcel() {
  ...
  const data = [];
  const headers = ["Id", "Code", "Name", "Type", "Description", "Date Recorded"];
  ...
  selectedRows.forEach((row) => {
    ...
    const description = tableRow.querySelector("td:nth-child(7)").textContent.trim();
    const dateRecorded = tableRow.querySelector(".created_at").textContent.trim();

    rowData.push(id, code, name, type, description, dateRecorded);
    data.push(rowData);
  });
  const worksheet = XLSX.utils.aoa_to_sheet(data);
  const workbook = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(workbook, worksheet, "Sheet1");
  XLSX.writeFile(workbook, "Allowance_data.xlsx");
}
...
searchInput.addEventListener('input', () => {
  ...
  tableRows.forEach(row => {
    ...
    const created_at = row.querySelector('td:nth-child(8)').textContent.toLowerCase();
...
function showModal(rowv) {
  ...
  const description = rowv.cells[6].textContent;
