...
function exportToExcel_archived() {
  ...
  XLSX.writeFile(workbook_archived, "Allowance_Archive.xlsx");
}
...
searchInput_archived.addEventListener('input', () => {
  ...
  tableRows_archived.forEach(row => {
    ...
    const description_archived = row.querySelector('td:nth-child(7)').textContent.toLowerCase();
    const deleted_at_archived = row.querySelector('td:nth-child(8)').textContent.toLowerCase();
...
function showModal_del(rowv_del) {
  ...
  const description_del = rowv_del.cells[6].textContent;
  const deleted_at_del = rowv_del.cells[7].textContent;
